...
public class Problems5 {
    public static void main(String[] args) {
        ...
        System.out.println(spiderVsFly("H4", "F3"));
        System.out.println(spiderVsFly("A4", "H2"));
        System.out.println(spiderVsFly("B4", "H4"));
        System.out.println(spiderVsFly("A2", "F1"));
    ...
    private static boolean sameLetterPattern(String a, String b){
        ...
        for(int i = 0; i < a.length() - 1; i++){
            int firstPattern = (int)a.charAt(i) - (int)(a.charAt(i + 1));
            int secondPattern = (int)b.charAt(i) - (int)(b.charAt(i + 1));
    ...
    public static String spiderVsFly(String x1, String x2){
        ...
        return move(startLevel, finishLevel, map, curIndex, goal, direction);
    }

    private static String move(int startLevel, int finishLevel, String map, int curIndex, int goalIndex, int direction){
        StringBuilder firstAnswer = new StringBuilder();
        StringBuilder secondAnswer = new StringBuilder();
        float firstLen = forwardMoving(firstAnswer, map, startLevel, finishLevel, curIndex, goalIndex);
        float secondLen = radianMoving(secondAnswer, map, startLevel, finishLevel, curIndex, goalIndex, direction);
        //System.out.println("first len - " + firstLen + " secondLen - " + secondLen);
        //System.out.println("first len - " + firstAnswer + " secondLen - " + secondAnswer);
        return firstLen > secondLen ? secondAnswer.toString() : firstAnswer.toString();
    }

    private static float forwardMoving(StringBuilder answer, String map, int startLevel, int finishLevel, int curIndex,
                                       int goalIndex){
        float tempLen = 0;
        float l = 1;
        while(startLevel != 0){
            answer.append(map.charAt(curIndex)).append(startLevel).append("-");
            startLevel--;
            tempLen += l;
        }
        answer.append("A0").append("-");
        startLevel++;
        while(startLevel != finishLevel + 1){
            answer.append(map.charAt(goalIndex)).append(startLevel).append("-");
            startLevel++;
            tempLen += l;
        }
    ...
    private static float radianMoving(StringBuilder answer, String map, int startLevel, int finishLevel, int curIndex,
                                      int goalIndex, int direction){
        float tempLen = 0;
        float l = 1;
        float side = (float)Math.sqrt(2*(l-Math.cos(Math.PI / 4)));
        if(startLevel >= finishLevel){
            ...
            while (curIndex != goalIndex){
                if(curIndex + direction < 0) curIndex = 8;
                curIndex = (curIndex + direction) % map.length();
                tempLen += side * startLevel;
                answer.append(map.charAt(curIndex)).append(startLevel).append("-");
            }
            answer.deleteCharAt(answer.length()-1);
        }
    ...
    public static int totalPoints(String[] words, String guessedWord){
        int points = 0;
        int superWords = 0;
        boolean isSuperWOrdFound = false;
        ...
        for(String word : words){
            ...
            if(isValid){
                if(word.equals(guessedWord)) isSuperWOrdFound = true;
                points += word.length() - 2;
                if(word.length() == 6) superWords++;
            }
        }
        return points + (isSuperWOrdFound ? superWords * 50 : 0);
    }
